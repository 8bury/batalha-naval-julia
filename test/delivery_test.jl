@testset "instruções e entrega" begin
    ...
    positioning = read(joinpath(root, "src", "views", "pages", "PositioningPage.jl"), String)
    ...
    @test !occursin("title_label(\"Batalha naval\")", battle)
    ...
    @test occursin("GtkScrolledWindow", positioning)
    @test occursin("page.halign = Gtk4.Align_FILL", positioning)
    @test occursin("content.halign = Gtk4.Align_FILL", positioning)
    @test !occursin("configure_container!", positioning)
    @test !occursin("page.width_request = 1180", positioning)
